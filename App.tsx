    ...
    <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-6">
        <div className="hidden md:flex items-center space-x-2">
          ...
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">easyScholar 实时检索中</span>
        </div>
        <a 
    ...
    <div className="h-screen overflow-hidden flex flex-col bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="flex-1 overflow-hidden max-w-7xl mx-auto w-full p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 flex flex-col space-y-6 overflow-hidden">
          <section className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-slate-200 relative overflow-hidden">
            ...
            <div className="absolute -right-10 -bottom-10 w-32 h-32 bg-indigo-600/20 rounded-full blur-3xl"></div>
          </section>
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col flex-1 overflow-hidden transition-all hover:shadow-md">
            <div className="grid grid-cols-2 gap-4 mb-5">
              <div>
                <label className="text-[10px] font-black text-slate-500 mb-2 block uppercase tracking-widest">AI 转换引擎</label>
              ...
              <div>
                <label className="text-[10px] font-black text-slate-500 mb-2 block uppercase tracking-widest">引注格式</label>
                <select 
                  value={citationStyle} 
            ...
            <button
              ...
              className={`mt-6 w-full py-4 rounded-2xl font-black text-white transition-all uppercase text-xs tracking-[0.2em] shadow-lg flex items-center justify-center space-x-2 ${isLoading ? 'bg-slate-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] shadow-indigo-100'}`}
            >
              {isLoading && (
              ...
              <span>{isLoading ? '正在进行全网等级监测...' : '立即转换并识别等级'}</span>
            </button>
          </section>
        </div>
        ...
        <div className="lg:col-span-7 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 rounded-2xl border border-slate-200 bg-white p-5 custom-scrollbar shadow-inner">
            {history.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-6 py-20">
                ...
                <div className="text-center">
                  <p className="text-xs font-black uppercase tracking-[0.3em] mb-1">等待处理文献</p>
                  <p className="text-[10px] opacity-60">请在左侧输入需要转换的原始引注</p>
                </div>
              </div>
            ) : (
              history.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    {item.rankDetail?.isNegative ? (
                      <span className="text-[9px] px-2.5 py-1 rounded bg-red-600 text-white font-black uppercase tracking-widest flex items-center ring-4 ring-red-600/10">
                    ...
                    {item.rankDetail?.tags?.map((tag, idx) => (
                      ...
                      >
                        ...
                        <i className="ml-1 opacity-50 text-[7px] font-black not-italic">官</i>
                      </span>
                    ))}
                  </div>
          ...
          <div className="mt-4 text-center text-[10px] text-slate-400 font-black uppercase tracking-[0.4em] py-2">
            点击结果条目即可一键复制到剪贴板
          </div>
        </div>
      </main>
      ...
      <footer className="bg-white text-slate-400 py-3 px-8 border-t border-slate-200 flex-shrink-0 z-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-bold tracking-widest uppercase">
          <div className="flex items-center space-x-3">
            <span className="text-indigo-600">LegalLink Core v10.2</span>
            <span className="opacity-30">|</span>
            <span className="text-slate-400">© 2025 法学引注转换系统</span>
          </div>
          <div className="flex flex-wrap justify-center gap-6 items-center">
            <div className="flex items-center space-x-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500"></span>
              <span>CLSCI / 法 C 扩 已集成</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="w-2 h-2 rounded-full bg-red-500"></span>
              <span>华政 2024 负面清单实时比对</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="w-2 h-2 rounded-full bg-slate-300"></span>
              <span>CSSCI 集刊官方库</span>
            </div>
          </div>
        </div>
      </footer>
