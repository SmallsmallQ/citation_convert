...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
      ...
      <main className="max-w-[1600px] mx-auto w-full p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 flex-1">
        {/* 输入面板 */}
        <div className="lg:col-span-4 flex flex-col space-y-4 lg:space-y-6">
          <section className="bg-slate-900 rounded-2xl p-5 lg:p-6 text-white shadow-xl shadow-slate-200 relative overflow-hidden animate-glow">
            <div className="relative z-10">
              <h2 className="text-lg lg:text-xl font-black mb-1.5 flex items-center tracking-wider">
                转换与等级识别
              </h2>
              <p className="text-slate-400 text-[10px] lg:text-xs leading-relaxed opacity-90 font-medium">
                支持 CLSCI、法C扩、CSSCI集刊、华政负面清单及全网预警名录。
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-bold py-2.5 px-3 focus:ring-2 focus:ring-indigo-500 text-slate-900 outline-none transition-all cursor-pointer hover:border-indigo-300"
                >
        ...
        <div className="lg:col-span-8 flex flex-col min-h-[400px]">
          <div className="flex items-center justify-between mb-4 px-1 lg:px-2">
          ...
          <div className="flex-1 space-y-4 lg:space-y-5">
            {history.length === 0 ? (
              <div className="bg-white border border-slate-200 border-dashed rounded-3xl h-full flex flex-col items-center justify-center text-slate-300 space-y-6 py-16 lg:py-24">
                <div className="w-16 h-16 lg:w-20 lg:h-20 bg-slate-50 rounded-full flex items-center justify-center border border-slate-100">
                  <svg className="w-8 h-8 lg:w-10 lg:h-10 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"/></svg>
                </div>
                <div className="text-center px-4">
                  <p className="text-[11px] font-black uppercase tracking-[0.3em] mb-1 text-slate-400">等待处理文献</p>
                  <p className="text-[10px] opacity-60">在左侧输入后，系统将实时调取 easyScholar 官方库进行等级标注</p>
                </div>
              </div>
            ) : (
              history.map((item) => (
            ...
            {history.length > 0 && (
              <div className="mt-8 mb-4 text-center text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em] py-2">
                点击上方结果条目即可一键复制
              </div>
            )}
          </div>
